 ...
 * @param callback 回调 参数：移动距离
 */
export const dragEle = (ele, callback) => {
    return (e) => {
        let target = ele || e.currentTarget;
        if (target.getAttribute('class') !== 'dragContainer') {
            return;
        }
        let clientX = e.clientX || e.targetTouches[0].clientX, //移动起点
            clientY = e.clientY || e.targetTouches[0].clientY; //移动起点
        //初始化组件定位位置
        const onMouseMove = (moveE) => {
                let left = parseInt(target.style.left) || 0,
                    top = parseInt(target.style.top) || 0,
                    offsetX = (moveE.clientX || moveE.targetTouches[0].clientX) - clientX,
                    offsetY = (moveE.clientY || moveE.targetTouches[0].clientY) - clientY,
                    moveX = left + offsetX,
                    moveY = top + offsetY;
                //以当前点为起点再次开始移动，以此类推，形成了移动
                clientX = offsetX + clientX;
                clientY = offsetY + clientY;
                //限制移动距离
                // if (move >= moveDistance) {
                //     move = moveDistance;
                // } else if (move < 0) {
...
/**
 * 获取元素相对于视口顶部，左边的固定距离，元素宽高
 * @param element
 * @returns {boolean}
 */
export const getDesToVP = (element) => {
    let rect = element.getBoundingClientRect();
    return {
 ...
 * 2分一个字符串
 * @param string
 * @param toNum 是否返回number类型
 * @returns {*[]}
 */
export const divs2 = (string, toNum = true) => {
    let value1 = string.slice(0, Math.ceil(string.length / 2)),
        value2 = string.slice(Math.ceil(string.length / 2));
    if (toNum) {
 ...
 * @param callback
 */
export const traverseObj = (obj, callback) => {
    if (!isObject(obj)) {
        throw 'need a object param'
    }
    if (!isFunc(callback)) {
        throw 'need a func param'
    }
    for (let key in obj) {
        if (obj.hasOwnProperty(key)) {
