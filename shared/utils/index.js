 ...
 * @param path
 */
exports.pWriteFile = (path, data) => new Promise((resolve, reject) => {
    let fs = require('fs');
...
exports.checkCollection = async(docList, uid, token, baseDoc) => {
    let msgList = [], ids = [], findList = [];
    for (let i = 0; i < docList.length; i++) {
        ids.push({uid, entryId: docList[i]._id});
        msgList.push(docList[i].getObj);
    }
    if (token && ids.length > 0) {
        findList = await baseDoc.find({
            $or: ids
        }, 'entryId');
        for (let i = 0; i < msgList.length; i++) {
            for (let j = 0; j < findList.length; j++) {
                if (msgList[i].id == findList[j].entryId.toString()) {
                    msgList[i].userCollected = 1;
                    break;
                }
            }
        }
...
exports.checkThumbUp = async(docList, uid, token, baseDoc) => {
    let msgList = [], ids = [], findList = [];
    for (let i = 0; i < docList.length; i++) {
        ids.push({uid, commentId: docList[i]._id});
        msgList.push(docList[i].getObj);
    }
    if (token && ids.length > 0) {
        findList = await baseDoc.find({
        ...
        if (findList.length > 0) {
            for (let i = 0; i < msgList.length; i++) {
                for (let j = 0; j < findList.length; j++) {
                    if (msgList[i].id == findList[j].commentId.toString()) {
                        msgList[i].userThumbUped = 1;
                        break;
                    }
                }
            }
...
// const CODE = '151336';
// const KEYS = 'f l r u e';
// const getIps = () => new Promise((res, rej) => {
//     request
//         .post('http://67.218.152.253:8789/proxyIps')
//         .send({keys: enStr, type: 'h', pageNum: 0})
//         .end((err, sour) => {
//             if (err || !sour) {
//                 return rej(err);
//             }
//             if (sour.text.code === 0) {
//                 return res(sour.text.res)
//             } else {
//                 return res([])
//             }
//         });
